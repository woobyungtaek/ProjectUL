...
| 6   |   Odd : 좌표 합이 홀수 전체 (체크)       |  6   |   Odd : 좌표 합이 홀수 공격            (x : 가로 범위 , y : 세로 범위)
| 7   |   Even: 좌표 합이 짝수 전체 (체크)       |  7   |   Even: 좌표 합이 짝수 공격            (x : 가로 범위 , y : 세로 범위)
| 8   |   onlyStuct : 구조물이 있는 칸 전체      |  8   |   onlyStuct : 구조물이 있는 슬롯 공격  (x : 가로 범위 , y : 세로 범위)
| 9   |   onlyEnemy : 적이 잇는 칸 전체          |  9   |   onlyEnemy : 적이 잇는 슬롯 공격      (x : 가로 범위 , y : 세로 범위)
| 10  |   onlyEmpty : 빈칸 전체                  |  10  |   Random : 임의의 슬롯 공격            (x : 가로 범위 , y : 세로 범위) // 랜덤은 무기의 속성에 포함, 범위 내에서 무작위 선택하도록
